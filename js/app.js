...
(function () {
  ...
  let musicSrcLoaded = false;
  let noClickAttempts = 0;
  let noTextIndex = 0;

  const MUSIC_PATHS = ['/music/risk-it-all.mp3', '/assets/music/risk-it-all.mp3'];
  ...
  function loadMusicSource() {
    if (musicSrcLoaded || !bgMusic) return Promise.resolve(false);

    return new Promise((resolve) => {
      let index = 0;

      function tryNext() {
        if (index >= MUSIC_PATHS.length) {
          resolve(false);
          return;
        }

        const src = MUSIC_PATHS[index++];
        bgMusic.src = src;
  ...
  function fadeMusicTo(volume) {
    const target = volume;
    let vol = bgMusic.volume;
    const step = target > vol ? 0.03 : -0.03;
  ...
  async function playMusic() {
    ...
    if (!musicSrcLoaded) {
      const ok = await loadMusicSource();
      if (!ok) return false;
    }
  ...
  function persistCompatibilityResult() {
    ...
    if (window.ProposalStorage) {
      window.ProposalStorage.save({
        ...
        compatibilityMaxScore: result.maxScore,
  ...
  function persistApplicationData(extra = {}) {
    ...
    window.ProposalStorage.save(
      ...
      { notify: Boolean(extra.termsAccepted || extra.saidYes) }
    );
  }
